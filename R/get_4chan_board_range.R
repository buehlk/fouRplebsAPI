...
#' @description This function returns the threads from range of board page
...
get_4chan_board_range <- function(board, page_start, page_stop,
                                  latest_comments = TRUE, cool = 5){
  range <- page_start:page_stop
  do.call("rbind", lapply(range, function(i){
    get_4chan_board_index(board = board, page = i,
                          latest_comments = latest_comments, cool = cool)
    }
    ))
}
