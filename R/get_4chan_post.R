...
get_4chan_post <- function(board, post_id) {
  ...
  data.frame("thread_id" = parsed$thread_num,
             ...
             "referencing_comment" = gsub(">>| ", "", stringr::str_extract(parsed$comment, ">>[0-9]*( |\\n)")),
             "comments" = gsub("@>>[0-9]*( |\\n)", "", parsed$comment),
             "poster_country" = ifelse(is.null(parsed$poster_country), NA, parsed$poster_country),
             "nreplies" = ifelse(is.null(parsed$nreplies), NA, parsed$nreplies),
             "formatted" = parsed$formatted,
             "media_link" = ifelse(is.null(parsed$media), NA, parsed$media$media_link)
  )
}
