...
#' @param results String, Return post matches or the thread they are found in. Possible values: "all", "thread", Default: 'all'
...
#' @param cool Integer (seconds), The 4plebs API includes an API rate limit of 5 requests/min for the archive search. For multiple searches a cool-down is recommended , Default: 20
...
search_4chan <- function(start_date = "", end_date = "", boards, text = "", subject = "", filename = "", ghost = "all", user_id = "", tripcode = "", type = "all",
  ...
  print(paste("Approximate time:", round(ceiling(search_results_num/25)*20/60, 2), "minutes."))
